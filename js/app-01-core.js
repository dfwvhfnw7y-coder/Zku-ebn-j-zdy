...
function pinKey(n){if(pinEntry.length>=4)return;pinEntry+=n;for(var i=0;i<4;i++){var d=document.getElementById('pd'+i);d.classList.toggle('filled',i<pinEntry.length);d.classList.remove('error')}if(pinEntry.length===4){if(pinEntry===PIN_CODE){pinLocked=false;sessionStorage.setItem('pinOK','1');document.getElementById('pinOverlay').classList.add('hidden');setTimeout(function(){try{if(window.parent&&window.parent!==window)window.parent.location.reload();else window.location.reload()}catch(e){window.location.reload()}},60)}else{for(var j=0;j<4;j++)document.getElementById('pd'+j).classList.add('error');setTimeout(function(){pinEntry='';for(var k=0;k<4;k++){var d=document.getElementById('pd'+k);d.classList.remove('filled','error')}},600)}}}
